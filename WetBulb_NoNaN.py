...
def calc_RH_from_T_Td(T, Td, mode=0):
    ...
    if mode == 0: 
       ...
       Tdk = Td + SHR_CONST_TKFRZ # SHR_CONST_TKFRZ = 273.15
...
# Clausius-Clapeyron
    p_tmb = p_t*0.01 #pressure*.01
    tcfbdiff = T_k - Cf + beta # temp (K) - 273.15 + 243.5
    es_mb = es_C * np.exp(alpha*(T_k - Cf)/(tcfbdiff)) # 6.112 * exp(17.67(tempK-273.15))/(temp diff in line above)) = vapor pressure
    dlnes_mbdT = alpha * beta/((tcfbdiff)*(tcfbdiff)) # 6.112 * 243.5 divided by temp diff squared
    pminuse = p_tmb - es_mb # pressure (mb) - vapor pressure
    de_mbdT = es_mb * dlnes_mbdT # vapor pressure - 6.112 * 243.5 divided by temp diff
    d2e_mbdT2 = dlnes_mbdT * (de_mbdT - 2*es_mb/(tcfbdiff)) # (6.112 * 243.5 divided by temp diff squared) * (vapor pressure - 6.112 * 243.5 divided by temp diff)\
                                                            # minus 2*vapor pressure all divided by temp diff

# Constants used to calculate rs(T)
    ndimpress = (p_tmb/refpres)**vkp #(pressure(mb)/1000)^0.2854
    p0ndplam = refpres * ndimpress**lambd_a #1000*((pressure/1000)^0.2854)^3.504
    rs = epsilon * es_mb/(p0ndplam - es_mb + np.spacing(1)) #eps) # 0.6220 * (vapor pressure)/1000*((pressure/1000)^0.2854)^3.504 (rounded)
    prersdt = epsilon * p_tmb/((pminuse)*(pminuse)) #0.6220 
...
# avoid bad numbers
    ...
    ## this is what the new code should be to reduce/eliminate NaNs
    rs[rs>1]=1
    rs[rs<0]=0
...
#    SHR_CONST_TKFRZ = 273.15
    ...
    # print('this is pressure ' + str(Pressure))

    es_mb,rs = QSat_2(TemperatureK, Pressure)[0:2] # first two returned values #goes to saturated mixing ratio function
    ...
    pnd = (pmb/p0)**(kappad) # p0=1000; pmb=Pressure*0.01; kappad=0.2854 (Heat Capacity) 
    ...
    theta_dl = T1*((p0/(pmb-vapemb))**kappad) * ((T1/tl)**(mixr*0.00028)) # T1 is TemperatureK #kappad = 0.2854 (Heat Capacity)
    # EPT: Bolton 1980 Eqn 39.  
    epott = theta_dl * np.exp(((3.036/tl)-0.00178)*mixr*(1 + 0.000448*mixr)) #mixr is specific humidity * 1000 #theta_dl is moist potential temp (K)
    Teq = epott*pnd			 # equivalent temp (K) at pressure #equivalent potential temp at pressure
    X = (C/Teq)**3.504 # (freezing temp/equivalent temp (K) ^ 3.504 (1/kappa))
